...
<form action="{{ route('detail-penjualans.store') }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <input type="number" name="jumlahproduk" id="jumlahproduk" class="form-control" value="{{ old('jumlahproduk') }}" readonly>
    </div>

    <div class="mb-3">
        <label for="subtotal" class="form-label">Subtotal</label>
        <input type="number" name="subtotal" id="subtotal" class="form-control" value="{{ old('subtotal') }}" readonly>
    </div>
...
<script>
    // Mengatur jumlah produk dan subtotal berdasarkan produk yang dipilih
    document.getElementById('produk_id').addEventListener('change', function () {
        const selectedOption = this.options[this.selectedIndex];
        const stock = selectedOption.getAttribute('data-stock');
        const price = selectedOption.getAttribute('data-price');

        // Isi jumlah produk dan subtotal
        document.getElementById('jumlahproduk').value = stock || 0;
        document.getElementById('subtotal').value = (price && stock) ? (price * stock).toFixed(2) : 0;
    });
</script>
</body>
</html>
